...
static bool CheckService(const uint256& proTxHash, const ProTx& proTx, CValidationState& state)
{
    ...
    if (Params().NetworkIDString() != "regtest" && !proTx.addr.IsRoutable()) {
        return state.DoS(10, false, REJECT_INVALID, "bad-protx-addr");
    }
...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        CCoins coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || ((coin.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_BASIC_COLLATERAL * COIN) &&
                                                            (coin.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_SUPER_COLLATERAL * COIN) &&
                                                            (coin.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_BAMF_COLLATERAL * COIN))) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }

        if (!ExtractDestination(coin.vout[ptx.collateralOutpoint.n].scriptPubKey, collateralTxDest)) {
        ...
        if (!IsValidDestination(collateralTxDest)) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-invalid-collateral-dest");
        }
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_BASIC_COLLATERAL * COIN &&
            tx.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_SUPER_COLLATERAL * COIN &&
            tx.vout[ptx.collateralOutpoint.n].nValue != ZELNODE_BAMF_COLLATERAL * COIN) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
...
bool CheckProUpRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (pindexPrev) {
        ...
        // don't allow reuse of collateral key for other keys (don't allow people to put the collateral key onto an online server)
        CTxDestination collateralTxDest;
        if (!ExtractDestination(coin.vout[dmn->collateralOutpoint.n].scriptPubKey, collateralTxDest)) {
...
std::string CProRegTx::MakeSignString() const
{
    ...
    if (ExtractDestination(scriptPayout, destPayout)) {
        if (IsValidDestination(destPayout)) {
            const CKeyID* keyID = boost::get<CKeyID>(&destPayout);
            if (!keyID) {
...
std::string CProRegTx::ToString() const
{
    ...
    return strprintf("CProRegTx(nVersion=%d, collateralOutpoint=%s, addr=%s, nOperatorReward=%f, ownerAddress=%s, pubKeyOperator=%s, votingAddress=%s, scriptPayout=%s)",
        nVersion, collateralOutpoint.ToString(), addr.ToString(), (double)nOperatorReward / 100, EncodeDestination(keyIDOwner), pubKeyOperator.ToString(), EncodeDestination(keyIDVoting), payee);
}
