...
void CDeterministicMN::ToJson(UniValue& obj) const
{
    ...
    if (GetUTXOCoin(collateralOutpoint, coin)) {
        CTxDestination dest;
        if (ExtractDestination(coin.vout[collateralOutpoint.n].scriptPubKey, dest)) {
...
bool CDeterministicMNManager::ProcessBlock(const CBlock& block, const CBlockIndex* pindex, CValidationState& _state, bool fJustCheck)
{
    ...
    bool fDIP0003Active = pindex->nHeight >= consensusParams.vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight;
    if (!fDIP0003Active) {
        return true;
    }
    ...
    // TODO. add the enforcemnet block hash at which DIP3 want to enforce
...
bool CDeterministicMNManager::UndoBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    const auto& consensusParams = Params().GetConsensus();
    if (nHeight == consensusParams.vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight) {
...
bool CDeterministicMNManager::BuildNewListFromBlock(const CBlock& block, const CBlockIndex* pindexPrev, CValidationState& _state, CDeterministicMNList& mnListRet, bool debugLogs)
{
    ...
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        ...
        if (tx.nType == TRANSACTION_PROVIDER_REGISTER) {
            ...
            CCoins coin;
            if (!proTx.collateralOutpoint.hash.IsNull() && (!GetUTXOCoin(dmn->collateralOutpoint, coin) || (coin.vout[dmn->collateralOutpoint.n].nValue != ZELNODE_BASIC_COLLATERAL * COIN) &&
                                                                                                                   (coin.vout[dmn->collateralOutpoint.n].nValue != ZELNODE_SUPER_COLLATERAL * COIN) &&
                                                                                                                   (coin.vout[dmn->collateralOutpoint.n].nValue != ZELNODE_BAMF_COLLATERAL * COIN))) {
    ...
    // we skip the coinbase
    for (int i = 1; i < (int)block.vtx.size(); i++) {
        auto tx = MakeTransactionRef(block.vtx[i]);
...
{
    if (tx->nVersion < DZN_TX_VERSION || tx->nType != TRANSACTION_PROVIDER_REGISTER) {
        return false;
    }
    CProRegTx proTx;
    if (!GetTxPayload(*tx, proTx)) {
...
bool CDeterministicMNManager::IsDIP3Enforced(int nHeight)
{
    ...
    return nHeight >= Params().GetConsensus().vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight;
}
...
void CDeterministicMNManager::UpgradeDBIfNeeded()
{
    ...
    if (chainActive.Height() < Params().GetConsensus().vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight) {
    ...
    curMNList.SetHeight(Params().GetConsensus().vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight - 1);
    curMNList.SetBlockHash(chainActive[Params().GetConsensus().vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight - 1]->GetBlockHash());

    for (int nHeight = Params().GetConsensus().vUpgrades[Consensus::UPGRADE_DIP0003].nActivationHeight; nHeight <= chainActive.Height(); nHeight++) {
